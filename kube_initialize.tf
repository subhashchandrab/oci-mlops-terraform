...
resource "null_resource" "get_model_ip_test" {
    ...
    provisioner "local-exec" {
        command = "TEST_MODEL_IP=$(kubectl get svc ml-model-service -n ml-model --no-headers=true | awk -F ' ' '{print $4}') && echo -n 'http://'$TEST_MODEL_IP':8080/predict' > data/test_model_predict_url.txt && echo -n 'http://'$TEST_MODEL_IP':8080/health' > data/test_model_health_url.txt"
        interpreter = [ "/bin/bash","-c"]
    }
}
...
resource "null_resource" "get_model_ip_prod" {
    ...
    provisioner "local-exec" {
        command = "PROD_MODEL_IP=$(kubectl get svc ml-model-service -n ml-model --no-headers=true | awk -F ' ' '{print $4}') && echo -n 'http://'$PROD_MODEL_IP':8080/predict' > data/prod_model_predict_url.txt && echo -n 'http://'$PROD_MODEL_IP':8080/health' > data/prod_model_health_url.txt"
        interpreter = [ "/bin/bash","-c"]
    }
}
